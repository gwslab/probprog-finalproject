...
def get_predictors(weather_df, intersection_df, categorical_mapping):
    ...
    for i in range(len(intersection_df)):
        if (
            intersection_df["nodes"][i] >= 0
            and intersection_df["nodes"][i] < predictors.shape[0]
        ):
            predictors[intersection_df["nodes"][i], :, 1] = np.log(
                intersection_df["Count_mean"][i]
            )
            predictors[intersection_df["nodes"][i], :, 2] = (
                intersection_df["num_connect"][i] >= 3
            )
