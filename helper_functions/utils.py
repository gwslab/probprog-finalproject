...
class FastCar(TorchDistribution):
    ...
    def __init__(self, alpha, tau, n, eigen, D_sparse, W_sparse):
        ...
        self.partial_sum = self.n * torch.log(self.tau) + torch.sum(
            torch.log(1 - self.alpha * self.eigen)
        )
        self._normal = dist.MultivariateNormal(torch.zeros(n), torch.eye(n))

        batch_shape = (
            torch.Size() if isinstance(alpha, Number) else self.alpha.size()
        )
    ...
    def log_prob(self, phi):
        ...
        phit_W[self.W_sparse[0]] = (
            phit_W[self.W_sparse[0]] + phi[self.W_sparse[1]]
        )
        phit_W[self.W_sparse[1]] = (
            phit_W[self.W_sparse[1]] + phi[self.W_sparse[0]]
        )
        result = (1 / 2) * (
            self.partial_sum
            + self.tau
            * (torch.sum(phit_D * phi) - self.alpha * torch.sum(phit_W * phi))
        )
        return result
