...
def plot_total_distributions(samples, train_mat, shape, subset=4):
    """
    Plots the  histplot of the total crashes per site on selected sites
    ...
    fig.suptitle(
        "Total crash posterior predictive distribution over period \
                of time at random sites"
    )
...
def plot_time_trend(samples, train_mat, window=61, polynomial=3):
    ...
    samples (ndarray): samples[i][j][k] number of crashes in node j on
    day k on posterior
        sample i.
    train_mat (ndarray): train_mat[i][j] crashes on site i on day j
