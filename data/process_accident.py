...
def filter_manhattan():
    ...
    manhattan_accidents.to_csv(
        "accident/processed/manhattan.csv"
    )  # manhattan_accidents_node_data.csv
...
def add_nearest_node():
    ...
    df = pd.read_csv(
        "accident/intermediate/clean.csv", index_col=0
    )  # reduced_data.csv
    ...
    df.to_csv(
        "accident/intermediate/with_node.csv", index=False
    )  # accident_node_data.csv
